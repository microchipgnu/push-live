...
step "AEO: platform-level discovery surface"
ROBOTS=$(curl -fsS "$BASE/robots.txt")
echo "$ROBOTS" | grep -q "GPTBot" || { red "/robots.txt missing GPTBot directive"; exit 1; }
echo "$ROBOTS" | grep -q "ClaudeBot" || { red "/robots.txt missing ClaudeBot directive"; exit 1; }
echo "$ROBOTS" | grep -q "PerplexityBot" || { red "/robots.txt missing PerplexityBot directive"; exit 1; }
echo "$ROBOTS" | grep -q "Sitemap: " || { red "/robots.txt missing Sitemap link"; exit 1; }
DOCSMD=$(curl -fsS "$BASE/docs.md")
echo "$DOCSMD" | grep -q "^# push-live" || { red "/docs.md missing top-level heading"; exit 1; }
LANDING=$(curl -fsS "$BASE/")
echo "$LANDING" | grep -q 'application/ld+json' || { red "landing missing inline JSON-LD"; exit 1; }
echo "$LANDING" | grep -q '"@type":"Organization"' || { red "landing missing Organization JSON-LD"; exit 1; }
DOCSHTML=$(curl -fsS "$BASE/docs")
echo "$DOCSHTML" | grep -q '"@type":"FAQPage"' || { red "/docs missing FAQPage JSON-LD"; exit 1; }
echo "$DOCSHTML" | grep -q '<link rel="canonical"' || { red "/docs missing canonical link"; exit 1; }
echo "$DOCSHTML" | grep -q '<meta name="description"' || { red "/docs missing meta description"; exit 1; }
green "  platform AEO surface: robots+ai-crawlers, /docs.md, landing+docs JSON-LD, canonical+meta"

step "AEO: hosted-site sidecars (forkable site as host)"
SROBOTS=$(curl -fsS "$BASE/s/$FSLUG/robots.txt")
echo "$SROBOTS" | grep -q "GPTBot" || { red "site /robots.txt missing AI crawler directives"; exit 1; }
echo "$SROBOTS" | grep -q "Auto-generated by push-live" || { red "site /robots.txt should be labeled auto-generated"; exit 1; }
SMAP=$(curl -fsS "$BASE/s/$FSLUG/sitemap.xml")
echo "$SMAP" | grep -q '<urlset' || { red "site /sitemap.xml malformed"; exit 1; }
echo "$SMAP" | grep -q '/s/'"$FSLUG"'/' || { red "site /sitemap.xml missing site URL"; exit 1; }
SLLMS=$(curl -fsS "$BASE/s/$FSLUG/llms.txt")
echo "$SLLMS" | grep -q "Base URL:" || { red "site /llms.txt missing Base URL"; exit 1; }
echo "$SLLMS" | grep -q "Sitemap:" || { red "site /llms.txt missing Sitemap line"; exit 1; }
SCARD=$(curl -fsS "$BASE/s/$FSLUG/.well-known/agent-card.json")
echo "$SCARD" | python3 -c "import json,sys;d=json.load(sys.stdin);assert d['discovery']['sitemap'];assert d['discovery']['llms_txt'];assert d['hosted_by']['name']=='push-live'" || { red "site agent-card.json missing required fields"; exit 1; }
SMD=$(curl -fsS "$BASE/s/$FSLUG/index.md")
echo "$SMD" | grep -qi "fork me" || { red "site /index.md should mirror /index.html content"; exit 1; }
green "  hosted-site sidecars: robots+sitemap+llms.txt+agent-card+index.md all served"
...
step "AEO: gated site sidecars don't leak content"
# /llms.txt: must serve (so crawlers know the site exists) but mark gated.
GLLMS=$(curl -fsS "$BASE/s/$PSLUG/llms.txt")
[[ "$GLLMS" == *"Access-gated"* ]] || { red "gated site /llms.txt missing access-gated marker (got: $GLLMS)"; exit 1; }
[[ "$GLLMS" != *"<h1>paid</h1>"* ]] || { red "gated site /llms.txt leaked page body"; exit 1; }
# /llms-full.txt: must serve, must NOT contain the body, must include the omitted notice.
GFULL=$(curl -fsS "$BASE/s/$PSLUG/llms-full.txt")
[[ "$GFULL" == *"[Full content omitted"* ]] || { red "gated site /llms-full.txt missing omission notice"; exit 1; }
[[ "$GFULL" != *"<h1>paid</h1>"* ]] || { red "gated site /llms-full.txt leaked page body"; exit 1; }
# /index.md mirror: must NOT serve for gated sites (auto-generated path returns null → 404/non-md).
MDCODE=$(curl -s -o /dev/null -w '%{http_code}' -H 'accept: text/markdown' "$BASE/s/$PSLUG/index.md")
[[ "$MDCODE" != "200" ]] || {
  # If it does 200, it must NOT be the page body — verify.
  MDBODY=$(curl -fsS "$BASE/s/$PSLUG/index.md")
  [[ "$MDBODY" != *"paid"* ]] || { red "gated site /index.md leaked page body"; exit 1; }
}
green "  gated site: llms.txt marked, llms-full.txt omitted, index.md mirror suppressed"
